...
3. Update the matching row of `LAYOUT` in `tests/app/governance/test_policy.py` and the row of
...
- `tests/unit/tools/test_start.py` × 3: `KeyError: 'business_outcomes'` at `tools/start.py:351`,
  which still reads `draft["contract"]["outcomes"]` and appends to `keep_outcomes`.
  That is the rename, not yet applied to this file.
- `tests/unit/governance/test_store.py::test_a_run_compiled_after_it_moved_still_finds_its_crops`:
...
- `tests/app/replay/test_engine.py` × 2 (`transient_error_is_retried`,
