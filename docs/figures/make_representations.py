...
W, H = 1240, 640
PANEL_W, PANEL_H, GAP, TOP = 372, 470, 34, 96
out = []
add = out.append
...
x = xs[0]
img_w = PANEL_W - 32
img_h = int(img_w * 800 / 1100)
add(f'<image href="{b64(shot)}" x="{x + 16}" y="{TOP + 56}" width="{img_w}" height="{img_h}" '
...
for line in observed["controls"].splitlines()[:12]:
    line = line if len(line) <= 58 else line[:57] + "…"
    chosen = line.startswith("[1]")
    if chosen:
        add(f'<rect x="{x + 10}" y="{y - 11}" width="{PANEL_W - 20}" height="16" rx="3" '
            f'fill="#ffffff" stroke="{BLUE}" stroke-width="1"/>')
    text(x + 16, y, line, size=9.5, font=MONO, fill=INK if line.strip() else MUTED,
...
for i, label in enumerate(("the model reads both", "code records, never the model")):
    ...
    text((x1 + x2) / 2, TOP - 10, label, size=8.5, fill=MUTED, anchor="middle")
